...
static void MakeStructArray(struct ArrowArray* array, const int32_t* values, int64_t n) {
...
static void MakeDictionaryArray(struct ArrowArray* array) {
  // Create indices
  ASSERT_EQ(ArrowArrayInitFromType(array, NANOARROW_TYPE_INT32), NANOARROW_OK);
...
TEST(ArrayMoveSharedTest, MoveArrayWithChildren) {
  ...
  const int32_t* data =
      reinterpret_cast<const int32_t*>(shared.children[0]->buffers[1]);
  EXPECT_EQ(data[0], 10);
  EXPECT_EQ(data[2], 30);
